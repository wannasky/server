...
class HttpServer {
    ...
    constructor(options) {
        ...
        this.tryFiles = options.try_files;
    ...
    pipeStatic() {
        this.before.push((req, res) => {
            ...
            url = path.extname(url) ? url : url + '/index.html';
            if (this.liveReload && (url === this.entry)) {
                const method = this.memory ? 'readMemoryFileSync' : 'readFileSync';
                const $ = cheerio.load(fileUtil[method](path.join(process.cwd(), this.root, url)));
                ...
                if (url === '/livereload.js') {
                    ...
                    if (this.tryFiles) {
                        const method = this.memory ? 'existMemory' : 'exist';
                        let fileUrl = url;
                        let query = url.indexOf('?');
                        if(query !== -1) fileUrl = fileUrl.slice(0,query);
                        if (!fileUtil[method](path.join(process.cwd(), this.root, fileUrl))) {
                            res.renderUrl(path.join(process.cwd(), this.root, this.tryFiles), this.memory, fileUtil.mfs);
                            return;
                        }
                    }
                    res.renderUrl(path.join(process.cwd(), this.root, url), this.memory, fileUtil.mfs);
                }
            }
        });
    }
